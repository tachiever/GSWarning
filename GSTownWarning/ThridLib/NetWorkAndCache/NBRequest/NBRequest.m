...
+ (void)requestWithURL:(NSString *)url type:(NBRequestType)type success:(void (^)(NSData *data))success failed:(void (^)(NSError *error))failed {
    ...
    if (((NetWorkStatus == AFNetworkReachabilityStatusNotReachable && [cacheManager isFileExistWithUrl:url]) ||
         (NetWorkStatus == AFNetworkReachabilityStatusReachableViaWWAN && [cacheManager isFileExistWithUrl:url] && [cacheManager isOutTimeWithUrl:url time:30*60] == NO) || (NetWorkStatus == AFNetworkReachabilityStatusReachableViaWiFi && [cacheManager isFileExistWithUrl:url] && [cacheManager isOutTimeWithUrl:url time:30*60] == NO))  && (type != RequestRefresh)) {
        // 读取缓存
        // NSLog(@"从缓存去读");
